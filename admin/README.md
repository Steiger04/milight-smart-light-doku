# Konfiguration
Voraussetzung für die Konfiguration ist eine vorhandene Instanz des miligt-smart-light Adapters. Die Erstellung einer
...
### Typ
Der Typ legt den Typ der Bridge fest. Mögliche Werte sind Legacy und Version 6. Legacy ist der alte Typ, der durch
die Version 6 abgelöst wurde.

### IP
IP der Bridge. Über den blauen Button wird automatisch nach Bridges im Netzsegment des Adapters gesucht. Das Ergebnis
wird in einer Auswahl angezeigt. <img :src="$withBase('/images/adapterconfig-ipauswahl.png')" class="my-img">
...
zwischen Bridge und Accesspoint hin. Wertänderungen sollten immer nur in einser Schritten vollzogen werden.
...
Millisekunden (5-10) schon etwas bewirken.
:::
...
können (auch FUT035 - FUT039 mit Upgrade) und nicht mehr mit der alten Mi-Light 3.0 App angelernt werden können.

## ZONEN
<img :src="$withBase('/images/adapterconfig-zonen-leer.png')" class="my-img">

Alle Pflichtfelder sind rot markiert. Falsche Eingaben werden soweit möglich abgefangen. Erst wenn alle Eingaben korrekt
erfolgt sind (keine roten Markierungen mehr), ist eine Speicherung möglich. Dies gilt für alle Eingaben im 
milight-smart-light Admin.
...
|-------|---|
|**legacy**|**4 Zonen**|
|**iBox1**|**4(+1) Zonen o. 8(+1) Zonen**|
|**iBox2**|**4 Zonen o. 8Zonen**|

::: warning HINWEIS
(+1) entspricht der Zone der iBox1 eigenen Bulb. Bei der 4 Zonen Auswahl muss dies immer Zone 5 (4 + 1) und bei der 8
Zonen Auswahl muss dies immer Zone 9 (8 + 1) sein.
:::
...
hier vorsichtig nachregulieren. <b>Der default Farboffset ist 0x48</b>. Ich habe bei meinen Strips damit sehr
gute Erfahrungen gemacht und konnte so die betroffenen Zonen individuell nachregulieren.
...
### Raum und Gruppe(n)
Her erfolgt die Zuordung zu Raum und Gruppe(n). Jede Zone kann einem Raum und mehreren Gruppe(n) zugeordnet werden.
Das Anlegen von Räumen und Gruppen erfolgt im ioBroker Admin unter Aufzählungen. Dabei entsprechen die Funktionen
den Gruppe(n).
::: warning HINWEIS
Die Zuordnung muß für den [IoT](/iot/) Adapter erfolgen. Nur dann wird unter **ALEXA-GERÄTE** ein Gerät automatisch
 erstellt
, wenn zuvor unter **RÄUME UND FUNKKTIONEN** Raum und Gruppe(n) der Zone aktiviert worden ist.
...
Erst nach Aktivierung der App kann diese genutzt werden. Hierzu kann im ioBroker Admin unter Instanzen auf das
Icon **Adapter-Webseite öffnen** einer der angelegten milight-smart-light Instanzen geklickt werden.
 
<img :src="$withBase('/images/adapterconfig-appstart.png')" class="my-img">

::: danger WICHTIG
Werden mehrere Instanzen angelegt, muß die App nur einmal aktiviert werden. Eine Mehrfachaktivierung unter dem selben
**Server Port** ist nicht möglich, da die Instanz während der Aktivierung überprüft, ob der **Server Port**
...
### Server Port
Der Port unter dem der Webserver des Adapters erreicht wird. Der Server Port ist Bestandteil der URL zum Aufrufen
...
### Socket Port
Die Kommunikation zwischen der App und dem ioBroker läuft bidiretional und in Echtzeit über einen Socket. Der **Socket
Port** ist mit **8082** vorbelegt. Dies ist der Standard Port des Web Adapters. Soll der Web Adapter auch für die Socket
Kommunikation genutzt werden, ist zwingend **integriert** auszuwählen. Ist der Port des Web Adapters nicht 8082, so muß
auch die Vorbelegung des Socket Ports auf den neuen Port des Web Adapters **angepasst** werden.
...
Wesentlichen abhängig von der Geschwindigkeit des WLAN's und des eingesetzten ioBroker Servers (Pi, NAS, NUC, etc.).
...
Der Default Wert für die Entprellzeit ist 200 ms. Um zu entscheiden, ob 200 ms ein optimaler Wert für die
...
Der nachfolgende Screenshot zeigt das Log für eine Entprellzeit von 200 ms. Dieser Wert kann für die hier genutzte
...
Im Gegesatz hierzu zeigt der nächste Screenshot das Log für eine Entprellzeit von 120 ms. Hier ist deutlich das
